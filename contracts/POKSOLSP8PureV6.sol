...
/**
 * @title POKSOLSP8PureV6
 * @dev Pure LSP8 contract for POKSO Cyberpunk V6 - Owner-only minting (no payments)
 * Standard LSP8 with proper Universal Profile compatibility
 * V6: Free mint version
 */
contract POKSOLSP8PureV6 is LSP8IdentifiableDigitalAsset {
    ...
    function mintTo(address recipient) external onlyOwner returns (bytes32 tokenId) {
        require(currentTokenId < MAX_SUPPLY, "POKSOLSP8PureV6: max supply reached");
    ...
    function batchMintTo(address recipient, uint256 amount) external onlyOwner {
        require(currentTokenId + amount <= MAX_SUPPLY, "POKSOLSP8PureV6: not enough supply");
